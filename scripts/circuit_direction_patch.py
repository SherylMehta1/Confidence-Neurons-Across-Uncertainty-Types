...
def main():
    ...
    guard_output(out_dir / "direction_patch.csv", args.overwrite)
    ...
    df.to_csv(out_dir / "direction_patch.csv", index=False)
    ...
    (out_dir / "direction_patch_summary.txt").write_text(summary + "\n", encoding="utf-8")
    write_provenance(out_dir / "direction_patch.csv", build_provenance(model, script="scripts/circuit_direction_patch.py", category=args.category, layers=layers, n_pairs=len(held), n_random=args.n_random))

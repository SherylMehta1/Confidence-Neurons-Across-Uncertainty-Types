...
SIG="${SIG-L31_N2477}"            # Phase-2 held-out survivor (Llama-3.1-8B); set SIG="" for other models
LAYER_RANGE="${LAYER_RANGE:-}"      # e.g. 16-27 for a 28-layer model (default: detect.py default 20-31)
SKIP_LEGACY_SETS="${SKIP_LEGACY_SETS:-0}"  # 1 = skip the Llama-specific old-15 / team-17 candidate sets
...
stage 30_detect     python scripts/detect.py ${LAYER_RANGE:+--layer-range $LAYER_RANGE} --overwrite --out results/candidate_neurons_bf16.json --distribution-out results/full_correlation_distribution_bf16.json
